...
10. **BFS 方向随机打乱**让 AI 行为不可预测，避免每次都走同一条路线
11. **去重处理**出现在多个地方（实体移动、渲染、放置），防止同一格上出现两个相同类型的实体（如两个 🅰 或两个 🅱）
12. **撤销系统保存完整状态克隆**而非操作栈，简化了回滚逻辑但内存开销较大（每个快照是整个棋盘 + 路径 + 规则）
13. **AI 利用已揭示规则更新认知**（`_getEffectiveProperty`）——即使某方自己没揭示，只要对方揭示了规则，AI 也能利用它来规划路径
14. **旗帜换位只发生在 AI 模式**——手动对战模式下旗帜位置保持不变，适合双人策略讨论
15. **旗帜在 `recomputeVisibility` 中享有永久可见权**——即使棋盘其他区域因 3 步记忆遗忘而陷入迷雾，旗帜格始终对双方可见，避免它"一会显示一会消失"
16. **内部 H/Z/N 字型墙体**——共 17 格连通墙块，分布在棋盘不同区域，AB 均无法推动，增加地图策略深度
17. **墙壁始终可见**——所有墙格即使未被揭示也显示 🧱 图标（暗雾态），玩家无需探索就能看到棋盘地形骨架，但墙内实体仍受迷雾保护
16. **无外围墙**——棋盘最外圈不再是墙壁，玩家可以走到边缘格，边界由引擎坐标检查保护
17. **原木暖色主题**——暖棕底色 + 陶土橙（A）+ 苔藓绿（B）+ 金棕点缀，替代旧版紫红冷色调
