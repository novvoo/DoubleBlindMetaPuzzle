...
**双盲元谜题** 是一款逻辑推理合作游戏。两位玩家（或 AI）共享同一个 11×9 棋盘，但各自拥有**完全不同的规则认知**。A 眼里的世界和 B 眼里的世界互不相通，只有通过**揭示隐藏规则**和**悬停联动**，才能逐步拼凑出真实世界的规则全貌，合作清除障碍。**最关键的规则——🚩旗帜=胜利——也必须通过探索发现！** 只有揭开 FLAG IS WIN 规则的玩家才能让旗帜成为胜利条件。
...
| 🚩 | flag | 1 | 随机位置（排除 AB 起始及曼哈顿距离<5的区域 + 实体格） |
| 📜 | rule_block | 3 | 固定坐标 |
...
| (2, 8) | `FLAG IS WIN` | **关键规则！**揭示后旗帜对所有玩家变为 WIN |
...
- 🚩旗帜对双方初始均为未知属性——必须找到并揭示 (2,8) 格的 FLAG IS WIN 规则才能获胜
- 墙壁对所有玩家始终是 STOP
- **A 和 B 可以共处一格**，但**任何玩家都不能与动物（巴巴/凯凯）共处一格**——推动失败时移动也会失败
...
3. 然后调用 `recomputeVisibility(player)`：
   ...
   - **防线例外**：旗帜所在格始终对双方可见，不受 3 步遗忘影响
   - **暗雾例外**：暗雾格覆盖可见性，踏入消耗令牌并清除后方可见
4. 结果：玩家能看到「当前位置」及「最近 3 步访问过的位置」这最多 15 个格子的十字可见区
5. 超过 3 步的旧位置自动退回到迷雾中，无法靠绕圈遍历拼出完整地图

**墙壁始终可见**：所有墙壁格子（含 H/Z/N 字型内部墙）始终在棋盘上显示 🧱 图标，即使该格未被任何玩家揭示。但墙壁格内的实体仍然受迷雾控制。

**可见格判定**：`state.isRevealedTo(player, x, y)` 检查 `cell.revealedTo` 是否包含该玩家。

**对方鬼影**：在迷雾格上如果对方玩家正好在那里，用 opacity 0.35 的半透明图标（🅰/🅱）显示一个鬼影。双方同格时图标自动缩小（14px）共享方格。
...
- 揭示后的规则进入**共识面板**，双方都可见
- 揭示消耗令牌，同时**自动切换回合**
- **令牌重生**：每位玩家每走 **10 步**自动获得 1 个揭示令牌（上限 5 个），确保游戏永不锁死
- **暗雾**：围绕旗帜生成迷雾区域。每 3 步及旗帜移动时清除旧迷雾并重新围绕旗帜生成（总计 18 格）。暗雾隐藏下方实体，踏入消耗 1 令牌并清除该格——持续消耗令牌，制造策略压力
...
- 任何一方踩上旗帜且该玩家知道 FLAG IS WIN → 立即获胜
- **FLAG IS WIN 并非双方起始规则**——双方必须通过探索找到 (2,8) 格并利用揭示令牌揭示该规则
- 一旦某方揭示 FLAG IS WIN，**双方 `playerRules` 都会同步更新**，双方均可通过踩旗帜获胜
- **踏入旗帜消耗 1 令牌**：揭示旗帜需要消耗令牌，令牌为 0 时旗帜视为 STOP，无法踏入
- 若 FLAG IS WIN 未被揭示，站在旗帜上不会触发胜利（旗帜只是一个无害的实体）
- 胜利后：AI 自动暂停，顶部显示半透明胶囊 Toast 提示，不弹窗遮挡棋盘
...
movePlayer(player, direction)
  ...
  ├── 生成事件列表：
  │   ├── { type: 'move', from, to }
  │   ├── { type: 'fog_cost', ... }（如果踏入暗雾）
  │   ├── { type: 'flag_cost', ... }（如果踏入旗帜）
...
AI 使用**6 级优先级链**，每步按序尝试，找到第一个可行行动即执行：

1. 🤫 **揭示规则**（站规则格上且有令牌时）
2. 🏁 **旗帜追逐**（已知 flag=win 且旗帜可见时才 BFS，flag 不明则不追）
3. 📜 **寻找未揭示规则**（扫描已揭示区，BFS 去最近的未揭示规则格）
4. 🌫 **迷雾探索**（向未揭示的迷雾边缘推进）
5. 🎲 **随机脱困**（stuck > 2 时随机尝试方向）
6. ⏳ **等待**（无路可走时跳回回合）
...
decide(player, lastFailedDir?)
  ...
  │       → 全部满足 → { action: 'reveal' }
  │
  ├── 2. 旗帜追逐（仅当已知 flag=win 时）
  │   └── _getEffectiveProperty(player, 'flag') === WIN ?
  │       └── 是 → _findVisibleFlag(player)
  │           └── 遍历所有已揭示格，查找 flag 实体
  ...
  │
  ├── 3. 寻找未揭示规则（关键策略）
  │   └── _findUnrevealedRuleCell(player, x, y)
  │       ├── 扫描已揭示区域内所有有规则但未揭示的格子
  │       ├── 按曼哈顿距离排序
  │       ├── BFS 尝试寻路到最近的可达未揭示规则格
  │       └── 找到路径 → { action: 'move', direction }
  │
  ├── 4. 迷雾探索
...
### `_getEffectiveProperty` —— 综合认知（共识优先）
...
**重要**：揭示规则后，`RevealSystem.revealRule()` 同时将该规则写入双方的 `playerRules`，因此 AI 的 `_getEffectiveProperty` 和引擎的 `_checkWin` 都能同步感知到新规则。这意味着一旦某方揭示了 FLAG IS WIN，双方都立即知道 flag=win，双方都可以通过踩旗帜获胜。
...
| `revealTokens` | `{ A: number, B: number }` | 剩余揭示令牌（每10步+1，上限5） |
| `playerStepCounts` | `{ A: number, B: number }` | 累计步数（用于令牌重生） |
| `revealedRules` | `RevealedRule[]` | 已揭示规则列表 |
| `darkFogCells` | `Set<"x,y">` | 暗雾格坐标集合 |
...
revealRule(player, x, y):
  ...
  6. 同步写入双方 playerRules（共识生效）
  7. 返回 { success, message, rule, x, y }
  
  令牌重生（stepAndCheckTokenReward）:
  每步 _advanceTurn 时调用 → playerStepCounts[player]++
  当 count % 10 === 0 → addRevealToken(player)（上限5）
```
...
recomputeVisibility(player):
  1. 扫描全棋盘，收集所有旗帜所在格 → flagCells[]
  2. 遍历所有格子，从 revealedTo 中删除该玩家
  3. 遍历 playerPath[player]（最近 3 步）：
     对每个位置 + 4 邻格 → revealTo(player, x, y)
  4. 遍历 flagCells[] → revealTo(player, x, y)
     （旗帜不受 3 步记忆遗忘影响，始终对双方可见）
```
...
| 旗帜换位目标 | 从候选空位随机 | AI 模式每10步 | `_relocateFlag()` |
...
| 面板 | 位置 | 内容 |
|------|------|------|
| **玩家状态** | 左侧 | A/B 玩家当前位置坐标 + 剩余揭示令牌 + 累计步数 + 彩色圆点 |
...
整体采用**原木暖色**风格 —— 模拟古旧木纹地牢在烛光下的质感，用低饱和度的暖棕色系打底，A/B 双方分别用暖橙（陶土）和冷绿（苔藓）区分，金色点缀规则和共识。
...
1. **双盲**意味着双方不知道对方的规则，也不知道关键规则（如 FLAG IS WIN）。只能通过揭示（共享共识面板）来逐步建立共同认知。**FLAG IS WIN 不在双方起始规则中**——必须先探索、找到规则方块、揭示它，才能让旗帜成为胜利条件
2. **避免死锁**：揭示令牌每 10 步自动重生（上限 5 个），确保即使初期用光令牌或无法触及规则格，游戏也不会陷入不可获胜的锁死状态
3. **暗雾压力**：暗雾始终围绕旗帜生成（总计 18 格，按旗帜数均分），每 3 步及旗帜移动时刷新。踏入暗雾消耗 1 令牌并清除该格。暗雾隐藏下方实体，制造持续的令牌消耗压力——玩家必须权衡：清暗雾还是留令牌揭示规则？
4. **3 步记忆**防止玩家靠反复移动来绘制完整地图，制造真正的信息不对称——最大可见面积（5格 × 3步）= 15格，而棋盘 99 格，永远看不到全局
5. **动态旗帜**在 AI 模式下每10步移动一次，迷雾也随之迁移。防止 AI 一次性找到旗帜后直接路径规划通关，需要持续探索
6. **A/B 互换**保证每次开局体验不同，消除固定位置带来的记忆优势
7. **鬼影机制**让迷雾中的对方位置半透明可见，避免完全盲猜对方在哪——这是「软信息」而非完整信息
8. **悬停联动**是给人工合作的沟通工具 —— 不需要说话也能指路
9. **STOP 优先级最高**的设计保证：即使一个格子上同时有 PUSH 和 STOP 实体，STOP 优先生效，玩家无法踏入
10. **玩家可共存，人兽不可共存**：A 和 B 可站在同一格，但玩家绝不能与巴巴、凯凯等动物共处一格。推动动物失败时（身后有墙或其他动物），整个移动也会失败，防止玩家"挤进"动物格
11. **BFS 方向随机打乱**让 AI 行为不可预测，避免每次都走同一条路线
12. **去重处理**出现在多个地方（实体移动、渲染、放置），防止同一格上出现两个相同类型的实体（如两个 🅰 或两个 🅱）
13. **撤销系统保存完整状态克隆**而非操作栈，简化了回滚逻辑但内存开销较大（每个快照是整个棋盘 + 路径 + 规则）
14. **AI 利用已揭示规则更新认知**（`_getEffectiveProperty`）——即使某方自己没揭示，只要对方揭示了规则，AI 也能利用它来规划路径
15. **旗帜换位只发生在 AI 模式**——手动对战模式下旗帜位置保持不变，适合双人策略讨论
16. **旗帜在 `recomputeVisibility` 中享有永久可见权**——即使棋盘其他区域因 3 步记忆遗忘而陷入迷雾，旗帜格始终对双方可见，避免它"一会显示一会消失"
17. **内部 H/Z/N 字型墙体**——共 17 格连通墙块，分布在棋盘不同区域，AB 均无法推动，增加地图策略深度
18. **墙壁始终可见**——所有墙格即使未被揭示也显示 🧱 图标（暗雾态），玩家无需探索就能看到棋盘地形骨架，但墙内实体仍受迷雾保护
19. **无外围墙**——棋盘最外圈不再是墙壁，玩家可以走到边缘格，边界由引擎坐标检查保护
20. **原木暖色主题**——暖棕底色 + 陶土橙（A）+ 苔藓绿（B）+ 金棕点缀，替代旧版紫红冷色调
