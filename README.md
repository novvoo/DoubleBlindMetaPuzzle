...
- **暗雾**：每 3 步全局随机生成 1-2 个暗雾格（上限 8 个）。暗雾覆盖已揭示格，隐藏下方实体，按 Q/U 消耗 1 令牌清除——持续消耗令牌，制造策略压力
...
| 旗帜换位目标 | 从候选空位随机 | AI 模式每5步 | `_relocateFlag()` |
...
| **键位说明** | 左侧 | WASD/IJKL 移动 + Q/U 清暗雾/揭示规则 + E/O 等待 + Ctrl+Z 撤销 |
...
3. **暗雾压力**：每 3 步全局在已揭示空地板格上生成暗雾（上限 8 个），消耗 1 令牌清除。暗雾隐藏下方实体，制造持续的令牌消耗压力——玩家必须权衡：清暗雾还是留令牌揭示规则？
4. **3 步记忆**防止玩家靠反复移动来绘制完整地图，制造真正的信息不对称——最大可见面积（5格 × 3步）= 15格，而棋盘 99 格，永远看不到全局
5. **动态旗帜**在 AI 模式下每5步移动一次，防止 AI 一次性找到旗帜后直接路径规划通关，需要持续探索
