...
3. **动态旗帜**在 AI 模式下每5步移动一次，防止 AI 一次性找到旗帜后直接路径规划通关，需要持续探索
...
9. **去重处理**出现在多个地方（实体移动、渲染、放置），防止同一格上出现两个相同类型的实体（如两个 🅰 或两个 🅱）
...
13. **旗帜在 `recomputeVisibility` 中享有永久可见权**——即使棋盘其他区域因 3 步记忆遗忘而陷入迷雾，旗帜格始终对双方可见，避免它"一会显示一会消失"
14. **内部 H/Z/N 字型墙体**——共 17 格连通墙块，分布在棋盘不同区域，AB 均无法推动，增加地图策略深度
15. **墙壁始终可见**——所有墙格即使未被揭示也显示 🧱 图标（暗雾态），玩家无需探索就能看到棋盘地形骨架，但墙内实体仍受迷雾保护
